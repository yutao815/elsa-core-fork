...
public class JsonRawHttpResponseContentReader : IHttpResponseContentReader
{
    ...
    public async Task<object> ReadAsync(HttpResponseMessage response, object context,
        CancellationToken cancellationToken) => await response.Content.ReadAsStringAsync();
}