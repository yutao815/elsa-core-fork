...
[Activity("Elsa", "HTTP", "Send HTTP Request.")]
public class SendHttpRequest : Activity
{
    private IEnumerable<IHttpResponseContentReader> _parsers;
    ...
    protected override async ValueTask ExecuteAsync(ActivityExecutionContext context)
    {
        ...
        var _httpClient = httpClientFactory.CreateClient(nameof(SendHttpRequest));

        _parsers = context.GetService<IEnumerable<IHttpResponseContentReader>>();
        ...
        {
            var formatter = SelectContentParser(context.Get(ResponseContentParserName), context.Get(ContentType));
    ...
    private IHttpRequestContentWriter SelectContentWriter(string contentType, IEnumerable<IHttpRequestContentWriter> _requestContentWriters)
    {
    ...
    private HttpRequestMessage PrepareRequest(ActivityExecutionContext context)
    {
        ...
        if (HasBodySupport(context.Get(Method)))
        {
    ...
    private bool HasBodySupport(string method)
    {
        var allowBodyMethods = new[] { "POST", "PUT", "PATCH" };
        return allowBodyMethods.Contains(method, StringComparer.CurrentCultureIgnoreCase);
    }
}
...
public static class MimeTypes
{
    ...
    public const string ApplicationXml = "xml";
